...
1. A `switch` statement is written with the following skeleton: `switch(){}`.
...
### Common Loops
1. `for(i = start; i < Array.length-1; i++){}` loops
    - Common Name: ***for-i*** loop
    - Note: `i` can be any variable name you would like to use.
      ...
      - When your variable is declared at the beginning of a *for-i* loop, it will only exist until the loop is finished. After the loop finishes, your variable will be empty.
    - `i` in this case, is an index number starting at `start`, and ending at `Array.length - 1`
    ...
    - `for ( initialization; condition; afterthought){codeblock}`
      - `initialization`
        - Creates a one-time use variable that will track the index of your loop. `i = 0` or `i = Array.length-1` (depending on where you want to start and end)
      ...
      - `afterthought`
        - This section of code will run *after* each time the loop runs.
        - Your `index` should always be incremented or decremented here.
...
2. `for(i of Array){}` loops
    - Common Name: ***for-of*** loop
    - Note: `i` in this case is **NOT** an `index`. Instead, `i` refers to an `item` within the `Array` while you loop though all items.
...
```
3. `Array.forEach( (element, index, Array) => {} )` method
    - Common Name: ***.forEach()*** method
    - Can only take a function as the argument
      - Nameless functions are fine as depicted above
    - Always attempts to pass a maximum of `3` parameters into the function
      - minimum of `1` parameter
...
- **Truthy** statements
  - Any statement that can be evaluated to `true` without explicitly being labeled as `true`
  - **Truthy examples**
    - A non-empty string: `let myString = 'value'`
    - A non-empty array: `const myArray = [1, 2]`
    - Any numerical value that is *not* zero `0`
- **Falsy** statements
  - Any statement that can be evaluated to `false` without explicitly being labeled as `false`
  - **Falsy examples**
    - An empty string: `let myString = ''`
      - Does *not* include a string with a space `' '`
    - An empty array: `const myArray = []`
  - Statements that are **falsy** by default
    - `null`
    - `undefined`
    - `false`
    - `0`
...
/*
  ...
  and because 3 is greater than 0, the statement is truthy
*/
```